...
########Input/Output Paths######################################################################################################################################
	...
	outDir='/mnt/nas-lab/OUTPUT/NOC/WorkinProgress'
...
########Bitrates`n Stuff################################################################
	...
	mailFrom="tc-205-HD@ott.volia.net"

#	time='-t 00:01:00'		####!Comment this line to process full duration!
...
################ffmpeg Transcoding##############################################################################################################################
		...
		ffmpeg -i "$f" \
		\
		-map 0:v -c:v libx264 -profile:v high -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid1bps}K -minrate ${vid1bps}K -maxrate ${vid1bps}K -bufsize $((2*$vid1bps))K -s 1920:1080 -aspect 16:9 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V1_${vid1bps}.ismv" \
		\
		-map 0:v -c:v libx264 -profile:v high -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid2bps}K -minrate ${vid2bps}K -maxrate ${vid2bps}K -bufsize $((2*$vid2bps))K -s 1920:1080 -aspect 16:9 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V2_${vid2bps}.ismv" \
		\
		-map 0:v -c:v libx264 -profile:v main -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid3bps}K -minrate ${vid3bps}K -maxrate ${vid3bps}K -bufsize $((2*$vid3bps))K -s 1280:720 -aspect 16:9 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V3_${vid3bps}.ismv" \
		\
		-map 0:v -c:v libx264 -profile:v main -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid4bps}K -minrate ${vid4bps}K -maxrate ${vid4bps}K -bufsize $((2*$vid4bps))K -s 1024:576 -aspect 16:9 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V4_${vid4bps}.ismv" \
		\
...
################bento4 Fragmentation############################################################################################################################
		...
		mp4fragment --timescale 10000000 --fragment-duration 2000 "${outPath}_V4_${vid4bps}.ismv" "${outPath}_V4_${vid4bps}_reF.ismv"
...
################Clean up########################################################################################################################################
		...
		mv -f "${outPath}_V4_${vid4bps}_reF.ismv" "${outPath}_V4_${vid4bps}.ismv"
...
################Creating Smooth Streaming manifests#############################################################################################################
		...
		ismindex -n "$outPath" \
		"${outPath}_V1_${vid1bps}.ismv" "${outPath}_V2_${vid2bps}.ismv" "${outPath}_V3_${vid3bps}.ismv" "${outPath}_V4_${vid4bps}.ismv" \
...
################Finalizing######################################################################################################################################
		...
		echo "$assName" >> /var/log/HD4VoD.txt
		
	done	############For each file in watchfolder LOOP END

	myDate=$(echo $(date  "+%T | %d.%m.%y"))
	mail -s "VoD transcoding complete @ $myDate" -r "$mailFrom" "$mailTo" < /var/log/HD4VoD.txt
