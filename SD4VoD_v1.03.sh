...
########Bitrates`n Stuff################################################################
	...
	mailFrom="tc-205-SD@ott.volia.net"
...
################ffmpeg Transcoding##############################################################################################################################
		...
		ffmpeg -i "$f" \
		\
		-map 0:v -c:v libx264 -profile:v main -level:v 3.0 -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid1bps}K -minrate ${vid1bps}K -maxrate ${vid1bps}K -bufsize $((2*$vid1bps))K -s 720:576 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V1_${vid1bps}.ismv" \
		\
		-map 0:v -c:v libx264 -profile:v main -level:v 3.0 -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid2bps}K -minrate ${vid2bps}K -maxrate ${vid2bps}K -bufsize $((2*$vid2bps))K -s 720:576 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V2_${vid2bps}.ismv" \
		\
		-map 0:v -c:v libx264 -profile:v main -level:v 3.0 -preset medium -x264-params "nal-hrd=cbr:force-cfr=1" -b:v ${vid3bps}K -minrate ${vid3bps}K -maxrate ${vid3bps}K -bufsize $((2*$vid3bps))K -s 720:576 -coder 1 -bf -1 -refs 4 -pix_fmt yuv420p -g 50 -keyint_min 50 -sc_threshold 0 -movflags frag_keyframe $time -y "${outPath}_V3_${vid3bps}.ismv" \
		\
